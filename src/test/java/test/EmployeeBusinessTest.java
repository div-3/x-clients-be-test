...
@ExtendWith({CompanyServiceResolver.class, HiberSessionResolver.class, HiberEmployeeRepositoryResolver.class, HiberCompanyRepositoryResolver.class})
public class EmployeeBusinessTest {
    ...
    public void createTest(EmployeeRepository employeeRepository, CompanyRepository companyRepository) throws SQLException {
        int compId = companyRepository.create("Test1");
        CompanyEntity company = companyRepository.getById(compId);
        EmployeeEntity employeeEntity = new EmployeeEntity(1, true, Timestamp.valueOf(LocalDateTime.now()), Timestamp.valueOf(LocalDateTime.now()), "Иван", "Д", "В", "89246010000", "div@mail.ru", null, null, company);
        int id = employeeRepository.create(employeeEntity);
        System.out.println("Создан сотрудник: id = " + id + "Данные: " + employeeEntity.toString());
    }
